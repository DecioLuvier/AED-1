...
int fourSumCount(int* nums1, int nums1Size, int* nums2, int nums2Size, int* nums3, int nums3Size, int* nums4, int nums4Size) {
    int tamanho = nums1Size;
    int totalPares = tamanho * tamanho;
    int* somas1_e_2 = (int*)malloc(totalPares * sizeof(int));
    int* somas3_e_4 = (int*)malloc(totalPares * sizeof(int));
	// Preencher com todas as somas possíveis
	GerarSomasDePares(somas1_e_2, nums1, nums2, tamanho);
    GerarSomasDePares(somas3_e_4, nums3, nums4, tamanho);
    // Ordenar para procurar nas pontas.
    qsort(somas1_e_2, totalPares, sizeof(int), CompararInt);
    qsort(somas3_e_4, totalPares, sizeof(int), CompararInt);
    // Sliding window
    int totalCombinacoesZero = 0;
    int ponteiroEsquerda = 0;                  
    int ponteiroDireita = totalPares - 1;      

    while (ponteiroEsquerda < totalPares && ponteiroDireita >= 0) {
        int somaAtual = somas1_e_2[ponteiroEsquerda] + somas3_e_4[ponteiroDireita];
        if (somaAtual == 0) {
            // Deu zero mas podem haver números repetidos nas listas.
            int repeticoesEsquerda = 1;
            while (ponteiroEsquerda + 1 < totalPares && somas1_e_2[ponteiroEsquerda] == somas1_e_2[ponteiroEsquerda + 1]) {
                repeticoesEsquerda++;
                ponteiroEsquerda++; 
            }
            int repeticoesDireita = 1;
            while (ponteiroDireita - 1 >= 0 && somas3_e_4[ponteiroDireita] == somas3_e_4[ponteiroDireita - 1]) {
                repeticoesDireita++;
                ponteiroDireita--;
            }
            totalCombinacoesZero += (repeticoesEsquerda * repeticoesDireita);
            ponteiroEsquerda++;
            ponteiroDireita--;
        } 
        else if (somaAtual < 0) 
